...
namespace BeezMaster\QLSClient\ValueObjects;
...
abstract readonly class AbstractValueObject implements Arrayable
{
     ...
     *@param array<string, mixed> $data
     */
    protected static function parseAsNullableBoolean(array $data, string $key): ?bool
    {
     ...
     *@param array<string, mixed> $data
     */
    protected static function parseAsEnum(array $data, string $key, string $enumClass): ?BackedEnum
    {
        if (!$value = self::parseAsNullableString($data, $key)) {
            return null;
        }
